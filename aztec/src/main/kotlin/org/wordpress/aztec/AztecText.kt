...
open class AztecText : AppCompatEditText, TextWatcher, UnknownHtmlSpan.OnUnknownHtmlTappedListener,
	IEventInjector {
	companion object {
		const val BLOCK_EDITOR_HTML_KEY = "RETAINED_BLOCK_HTML_KEY"
		const val BLOCK_EDITOR_START_INDEX_KEY = "BLOCK_EDITOR_START_INDEX_KEY"
        const val BLOCK_DIALOG_VISIBLE_KEY = "BLOCK_DIALOG_VISIBLE_KEY"

        const val LINK_DIALOG_VISIBLE_KEY = "LINK_DIALOG_VISIBLE_KEY"
        const val LINK_DIALOG_URL_KEY = "LINK_DIALOG_URL_KEY"
        const val LINK_DIALOG_ANCHOR_KEY = "LINK_DIALOG_ANCHOR_KEY"
        const val LINK_DIALOG_OPEN_NEW_WINDOW_KEY = "LINK_DIALOG_OPEN_NEW_WINDOW_KEY"

        const val HISTORY_LIST_KEY = "HISTORY_LIST_KEY"
        const val HISTORY_CURSOR_KEY = "HISTORY_CURSOR_KEY"

        const val SELECTION_START_KEY = "SELECTION_START_KEY"
        const val SELECTION_END_KEY = "SELECTION_END_KEY"

        const val INPUT_LAST_KEY = "INPUT_LAST_KEY"
        const val VISIBILITY_KEY = "VISIBILITY_KEY"
        const val IS_MEDIA_ADDED_KEY = "IS_MEDIA_ADDED_KEY"
        const val RETAINED_HTML_KEY = "RETAINED_HTML_KEY"
        const val RETAINED_INITIAL_HTML_PARSED_SHA256_KEY = "RETAINED_INITIAL_HTML_PARSED_SHA256_KEY"

        const val DEFAULT_IMAGE_WIDTH = 800

        val DEFAULT_ALIGNMENT_RENDERING = AlignmentRendering.SPAN_LEVEL

        var watchersNestingLevel: Int = 0

		private fun getPlaceholderDrawableFromResID(
			context: Context,
			@DrawableRes drawableId: Int,
			maxImageWidthForVisualEditor: Int
		): BitmapDrawable {
		...
		fun calculateInitialHTMLSHA(
			initialHTMLParsed: String,
			initialEditorContentParsedSHA256: ByteArray
		): ByteArray {
			try {
				// Do not recalculate the hash if it's not the first call to `fromHTML`.
				if (initialEditorContentParsedSHA256.isEmpty() || Arrays.equals(
	...
	private val REGEXP_EMAIL = Regex(
	...
	private fun init(attrs: AttributeSet?) {
		...
		setBackgroundResource(R.drawable.shape_editor_background)
		setTextColor(ContextCompat.getColor(context, R.color.text))
		setHintTextColor(ContextCompat.getColor(context, R.color.text_hint))

		drawableLoading =
			styles.getResourceId(R.styleable.AztecText_drawableLoading, R.drawable.ic_image_loading)
		drawableFailed =
			styles.getResourceId(R.styleable.AztecText_drawableFailed, R.drawable.ic_image_failed)
		...
		verticalParagraphMargin = styles.getDimensionPixelSize(
			R.styleable.AztecText_blockVerticalPadding,
			resources.getDimensionPixelSize(R.dimen.block_vertical_padding)
		)
		verticalHeadingMargin = styles.getDimensionPixelSize(
			R.styleable.AztecText_headingVerticalPadding,
	...
	override fun onRestoreInstanceState(state: Parcelable?) {
		...
		history.inputLast =
			InstanceStateUtils.readAndPurgeTempInstance<String>(INPUT_LAST_KEY, "", savedState.state)
		...
		val retainedHtml =
			InstanceStateUtils.readAndPurgeTempInstance<String>(RETAINED_HTML_KEY, "", savedState.state)
		...
		if (isBlockEditorDialogVisible) {
			...
			if (retainedBlockHtmlIndex != -1) {
				val unknownSpan = text.getSpans(
					retainedBlockHtmlIndex,
					retainedBlockHtmlIndex + 1,
					UnknownHtmlSpan::class.java
				).firstOrNull()
				if (unknownSpan != null) {
	...
	override fun onSaveInstanceState(): Parcelable {
		...
		val bundle = Bundle()
		InstanceStateUtils.writeTempInstance(
			context,
			externalLogger,
			HISTORY_LIST_KEY,
			ArrayList<String>(history.historyList),
			bundle
		)
		...
		if (addLinkDialog != null && addLinkDialog!!.isShowing) {
			...
			bundle.putString(
				LINK_DIALOG_OPEN_NEW_WINDOW_KEY,
				if (openInNewWindowCheckbox != null && openInNewWindowCheckbox.isChecked) "checked=true" else "checked=false"
			)
		}
	...
	fun getSelectedText(): String {
		if (selectionStart == -1 || selectionEnd == -1
			|| editableText.length < selectionEnd || editableText.length < selectionStart
		) return ""
	...
	fun contains(format: ITextFormat, selStart: Int = selectionStart, selEnd: Int = selectionEnd): Boolean {
		when (format) {
			...
			AztecTextFormat.FORMAT_HEADING_4,
			AztecTextFormat.FORMAT_HEADING_5,
			AztecTextFormat.FORMAT_HEADING_6 -> return lineBlockFormatter.containsHeading(
				format,
				selStart,
				selEnd
			)
			...
			AztecTextFormat.FORMAT_UNDERLINE,
			AztecTextFormat.FORMAT_STRIKETHROUGH,
			AztecTextFormat.FORMAT_CODE -> return inlineFormatter.containsInlineStyle(
				format,
				selStart,
				selEnd
			)
			AztecTextFormat.FORMAT_UNORDERED_LIST,
			AztecTextFormat.FORMAT_ORDERED_LIST -> return blockFormatter.containsList(
				format,
				selStart,
				selEnd
			)
			AztecTextFormat.FORMAT_ALIGN_LEFT,
			AztecTextFormat.FORMAT_ALIGN_CENTER,
			AztecTextFormat.FORMAT_ALIGN_RIGHT -> return blockFormatter.containsAlignment(
				format,
				selStart,
				selEnd
			)
			AztecTextFormat.FORMAT_QUOTE -> return blockFormatter.containsQuote(selectionStart, selectionEnd)
			AztecTextFormat.FORMAT_PREFORMAT -> return blockFormatter.containsPreformat(
				selectionStart,
				selectionEnd
			)
	...
	private fun loadImages() {
		...
		val loadingDrawable =
			AztecText.getPlaceholderDrawableFromResID(context, drawableLoading, maxImagesWidth)
	...
	private fun parseHtml(content: Spannable, withCursorTag: Boolean): String {
		...
		try {
			...
			AppLog.e(
				AppLog.T.EDITOR,
				"There was an error creating SpannableStringBuilder. See #452 and #582 for details."
			)
		...
		return EndOfBufferMarkerAdder.removeEndOfTextMarker(
			parser.toHtml(
				output,
				withCursorTag,
				shouldSkipTidying()
	...
	private fun switchToAztecStyle(editable: Editable, start: Int, end: Int) {
		editable.getSpans(start, end, IAztecBlockSpan::class.java)
			.forEach { blockFormatter.setBlockStyle(it) }
		editable.getSpans(start, end, EndOfParagraphMarker::class.java)
			.forEach { it.verticalPadding = verticalParagraphMargin }
		...
		for (span in urlSpans) {
			...
			editable.setSpan(
				linkFormatter.makeUrlSpan(span.url, span.attributes),
				spanStart,
				spanEnd,
		...
		codeSpans.forEach {
			...
			editable.setSpan(
				inlineFormatter.makeInlineSpan(it.javaClass, it.attributes),
				spanStart,
				spanEnd,
				Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
			)
	...
	fun removeBlockStylesFromRange(start: Int, end: Int, ignoreLineBounds: Boolean = false) {
		blockFormatter.removeBlockStyle(
			AztecTextFormat.FORMAT_PARAGRAPH,
			start,
			end,
			listOf(IAztecBlockSpan::class.java),
			ignoreLineBounds
		)
	}
		...
		when (id) {
			...
			clipboardIdentifier -> {
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N && Build.VERSION.SDK_INT < Build.VERSION_CODES.P
					...
					Toast.makeText(
						context,
						context.getString(R.string.samsung_disabled_custom_clipboard, Build.VERSION.RELEASE),
						Toast.LENGTH_LONG
					).show()
				} else {
	...
	fun copy(editable: Editable, start: Int, end: Int) {
		...
		val html =
			Format.removeSourceEditorFormatting(parser.toHtml(output), isInCalypsoMode, isInGutenbergMode)
		...
		if (clip != null) {
			...
			if (clip.itemCount > 0) {
				...
				val newHtml = oldHtml.replace(
					...
					textToPaste + "<" + AztecCursorSpan.AZTEC_CURSOR_TAG + ">"
				)
	...
	fun link(url: String, anchor: String, openInNewWindow: Boolean = false) {
		...
		if (TextUtils.isEmpty(url) && linkFormatter.isUrlSelected()) {
			...
			linkFormatter.editLink(
				url,
				anchor,
				openInNewWindow,
				linkFormatter.getUrlSpanBounds().first,
				linkFormatter.getUrlSpanBounds().second
			)
		} else {
	...
	fun showLinkDialog(
		...
		val openInNewWindow =
			if (TextUtils.isEmpty(presetOpenInNewWindow)) urlAndAnchor.third else presetOpenInNewWindow == "checked=true"
	...
	fun showBlockEditorDialog(unknownHtmlSpan: UnknownHtmlSpan, html: String = "") {
		...
		builder.setPositiveButton(R.string.block_editor_dialog_button_save) { _, _ ->
			...
			textBuilder.append(
				AztecParser(alignmentRendering, plugins).fromHtml(
					source.getPureHtml(),
					context
				).trim()
			)
			...
			val unknownClickSpan =
				text.getSpans(spanStart, spanStart + 1, UnknownClickableSpan::class.java).firstOrNull()
			...
			val newUnknownSpan =
				textBuilder.getSpans(0, textBuilder.length, UnknownHtmlSpan::class.java).firstOrNull()
	...
	fun removeMedia(attributePredicate: AttributePredicate) {
		text.getSpans(0, text.length, AztecMediaSpan::class.java)
			...
			.forEach {
				...
				val clickableSpan =
					text.getSpans(start, end, AztecMediaClickableSpan::class.java).firstOrNull()
